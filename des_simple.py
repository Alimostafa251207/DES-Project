...
def generate_round_keys(key_64: int) -> list:
    key_bits = int_to_bits(key_64, 64)
...
def des_encrypt_block(block_64: int, key_64: int) -> int:
    bits = int_to_bits(block_64, 64)
    bits = permute(bits, IP)
    L, R = bits[:32], bits[32:]
    round_keys = generate_round_keys(key_64)
    for k in round_keys:
        new_R = [l ^ f for l, f in zip(L, feistel_function(R, k))]
        L, R = R, new_R
    combined = R + L
    return bits_to_int(permute(combined, IP_INV))


def des_decrypt_block(block_64: int, key_64: int) -> int:
    bits = int_to_bits(block_64, 64)
    bits = permute(bits, IP)
    L, R = bits[:32], bits[32:]
    round_keys = generate_round_keys(key_64)
...
if __name__ == "__main__":
    ...
    print("DES Test Vector Verification")
    print("----------------------------")
    print(f"Plaintext : {plaintext:016X}")
    print(f"Key : {key:016X}")
    print(f"Ciphertext (Expected): {expected:016X}")
    print(f"Ciphertext (Computed): {ciphertext:016X}")
    print(f"Decrypted : {recovered:016X}")
    print("Encryption correct :", ciphertext == expected)
    print("Decryption correct :", recovered == plaintext)
